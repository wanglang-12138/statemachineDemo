...
public class FishAction implements Action<OrderStatus, OrderEvents> {

    @Override
    public void execute(StateContext<OrderStatus, OrderEvents> context) {
        // do something in every 1 sec
        System.out.println("用户：" + context.getStateMachine().getId() + " 的清蒸鱼正在制作中 | " + context.getEvent().getDescription());
    }
}
